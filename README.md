...
除了Web界面外，Neko AI还提供了功能强大的命令行界面。
...
#### 记忆管理
- `/clear_all` - 清除所有存储的记忆
- `/clear:关键词` - 清除包含特定关键词的记忆
- `/backup` - 备份当前所有记忆
- `/restore:路径` - 从指定路径恢复记忆
- `/optimize` - 优化记忆存储

#### 记忆可视化与查询
- `/stats` - 显示记忆统计信息
- `/search` 或 `/find` - 搜索记忆
- `/search:关键词` - 搜索包含特定关键词的记忆
- `/graph` - 可视化记忆图谱
- `/graph:关键词` - 可视化包含特定关键词的记忆图谱
- `/node` - 查看特定记忆节点的详细信息
- `/node:时间戳` - 查看指定时间戳的记忆节点

#### 系统维护
- `/logs:clear` - 清除日志文件
- `/init` 或 `/initialize` - 初始化系统检查
- `/export:` - 导出所有记忆为Markdown格式
- `/export:格式` - 导出所有记忆为指定格式 (markdown/json/txt)
- `/export:关键词` - 导出包含特定关键词的记忆
- `/export:格式 关键词` - 导出包含特定关键词的记忆为指定格式